...
constexpr uint32_t MATRIX_STATUS_HEARTBEAT_MS = 500;
constexpr gpio_num_t MATRIX_STATUS_RGB_PIN = GPIO_NUM_8;
constexpr uint16_t MATRIX_LAMP_PULSE_MIN_US = 100;
constexpr uint16_t MATRIX_LAMP_PULSE_STEP_US = 100;
constexpr uint16_t MATRIX_LAMP_PULSE_MAX_US = 600;
...
constexpr uint16_t MATRIX_SWITCH_SCAN_BLANK_US = 300;
constexpr uint16_t MATRIX_SWITCH_SCAN_SETTLE_US = 300;
constexpr uint16_t MATRIX_SWITCH_SCAN_RELEASE_US = 100;
constexpr uint8_t MATRIX_SWITCH_DEBOUNCE_TICKS = 4;
constexpr bool MATRIX_INTEGRATED_SWITCH_SCAN = false;
...
constexpr uint8_t MATRIX_DIAG_FORCE_ROW = 1;   // 0-based row index (1 => Row 2 / L19 row)
constexpr uint8_t MATRIX_DIAG_FORCE_COL = 4;   // 0-based lamp column index (4 => L19)
...
uint8_t switchSampleBits[CAPTAIN_SWITCH_BYTES] = {};
...
rmt_channel_handle_t matrixStatusLedChannel = nullptr;
rmt_encoder_handle_t matrixStatusLedEncoder = nullptr;
bool matrixStatusLedOn = false;
uint32_t matrixLastStatusHeartbeatMs = 0;
...
uint16_t appliedLampPulseWidthUs() {
    const uint16_t requested = static_cast<uint16_t>(
        MATRIX_LAMP_PULSE_MIN_US + static_cast<uint16_t>(lampPulseWidthLevel) * MATRIX_LAMP_PULSE_STEP_US);
    return (requested > MATRIX_LAMP_PULSE_MAX_US) ? MATRIX_LAMP_PULSE_MAX_US : requested;
}
...
void applySwitchDebounceFromSamples() {
    for (uint8_t sampleRow = 0; sampleRow < CAPTAIN_SWITCH_ROWS; sampleRow++) {
        for (uint8_t sampleCol = 0; sampleCol < CAPTAIN_SWITCH_COLS; sampleCol++) {
            const size_t bitIndex = captainSwitchBitIndex(sampleRow, sampleCol);
            const bool sampleClosed = captainGetBit(switchSampleBits, bitIndex);
...
void waitForRowSlotEnd(int64_t rowStartUs) {
    const int64_t rowBudgetUs = static_cast<int64_t>(MATRIX_ROW_BLANK_US) +
                                static_cast<int64_t>(MATRIX_ROW_SETTLE_US) +
                                static_cast<int64_t>(MATRIX_ROW_POST_HOLD_US) +
                                static_cast<int64_t>(appliedLampPulseWidthUs());
    const int64_t rowEndUs = rowStartUs + rowBudgetUs;
    while (esp_timer_get_time() < rowEndUs) {}
}

bool refreshLampMatrixStep() {
    ...
    const int64_t rowStartUs = esp_timer_get_time();
    ...
    if (!matrixSystemEnabled) {
        memset(switchSampleBits, 0, sizeof(switchSampleBits));
    ...
    if (MATRIX_INTEGRATED_SWITCH_SCAN && !MATRIX_DIAG_SKIP_SWITCH_SCAN) {
        for (uint8_t col = 0; col < CAPTAIN_SWITCH_COLS; col++) {
            const int level = gpio_get_level(static_cast<gpio_num_t>(CAPTAIN_MATRIX_SWITCH_COL_PINS[col]));
            const bool closed = (level == 0);
            captainSetBit(switchSampleBits, captainSwitchBitIndex(row, col), closed);
        }
    }

    if (!matrixOutputEnabled) {
        writeShiftRegister16(composeShiftFrame(0x00, 0x00));
        waitForRowSlotEnd(rowStartUs);
        row = static_cast<uint8_t>((row + 1) % CAPTAIN_SWITCH_ROWS);
        return row == 0;
    }
    ...
    if (colMask != 0) {
        ...
        // Spin on wall-clock timer instead of esp_rom_delay_us (cycle counter).
        // I2C slave interrupts advance the cycle counter while the ISR runs, causing
        // esp_rom_delay_us to exit early and shorten the column on-time for whichever
        // row is active when the control board's 15ms poll arrives. esp_timer_get_time()
        // reads a hardware counter unaffected by interrupt execution time.
        {
            const int64_t pulseEndUs = esp_timer_get_time() + appliedLampPulseWidthUs();
            while (esp_timer_get_time() < pulseEndUs) {}
        }
...
// Switch scanning is kept in a dedicated all-off pass so lamp refresh and switch
// sampling do not electrically interfere with each other.
void scanSwitchMatrix() {
    ...
    for (uint8_t row = 0; row < CAPTAIN_SWITCH_ROWS; row++) {
        writeShiftRegister16(composeShiftFrame(0x00, 0x00));
        esp_rom_delay_us(MATRIX_SWITCH_SCAN_BLANK_US);

        const uint8_t rowMask = static_cast<uint8_t>(1u << row);
        writeShiftRegister16(composeShiftFrame(rowMask, 0x00));
        esp_rom_delay_us(MATRIX_SWITCH_SCAN_SETTLE_US);
        ...
        writeShiftRegister16(composeShiftFrame(0x00, 0x00));
        esp_rom_delay_us(MATRIX_SWITCH_SCAN_RELEASE_US);
    }
...
bool writeStatusLedColor(uint8_t red, uint8_t green, uint8_t blue) {
    if (matrixStatusLedChannel == nullptr || matrixStatusLedEncoder == nullptr) {
        return false;
    }

    // WS2812 expects one pixel payload in GRB order.
    const uint8_t pixel[3] = {green, red, blue};
    rmt_transmit_config_t txConfig = {};
    txConfig.loop_count = 0;

    const esp_err_t txErr = rmt_transmit(
        matrixStatusLedChannel,
        matrixStatusLedEncoder,
        pixel,
        sizeof(pixel),
        &txConfig);
    return txErr == ESP_OK;
...
void initStatusHeartbeatLed() {
    ...
    txChannelConfig.gpio_num = MATRIX_STATUS_RGB_PIN;
    txChannelConfig.mem_block_symbols = 64;
    txChannelConfig.resolution_hz = 10 * 1000 * 1000;
    txChannelConfig.trans_queue_depth = 1;

    esp_err_t err = rmt_new_tx_channel(&txChannelConfig, &matrixStatusLedChannel);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "status LED channel init failed on GPIO%u: %s", static_cast<unsigned>(MATRIX_STATUS_RGB_PIN), esp_err_to_name(err));
        matrixStatusLedChannel = nullptr;
        return;
    }
    ...
    err = rmt_new_bytes_encoder(&bytesEncoderConfig, &matrixStatusLedEncoder);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "status LED encoder init failed: %s", esp_err_to_name(err));
        matrixStatusLedEncoder = nullptr;
        return;
    }

    err = rmt_enable(matrixStatusLedChannel);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "status LED RMT enable failed: %s", esp_err_to_name(err));
        return;
    }

    matrixStatusLedOn = false;
    matrixLastStatusHeartbeatMs = static_cast<uint32_t>(esp_timer_get_time() / 1000ULL);
    writeStatusLedColor(0, 0, 0);
    ESP_LOGI(TAG, "status LED ready on GPIO%u (WS2812 via RMT)", static_cast<unsigned>(MATRIX_STATUS_RGB_PIN));
}
...
void updateStatusHeartbeatLed() {
    ...
    const uint32_t nowMs = static_cast<uint32_t>(esp_timer_get_time() / 1000ULL);
    if ((nowMs - matrixLastStatusHeartbeatMs) < MATRIX_STATUS_HEARTBEAT_MS) {
        return;
    }

    matrixLastStatusHeartbeatMs = nowMs;
    matrixStatusLedOn = !matrixStatusLedOn;

    if (matrixStatusLedOn) {
        writeStatusLedColor(0, 24, 0);
    } else {
        writeStatusLedColor(0, 0, 0);
    }
}
...
void logLinkHeartbeat() {
    ...
    ESP_LOGI(TAG,
             ...
             " last_cmd=0x%02X pulse_us=%u lamp=[%02X,%02X,%02X,%02X,%02X] sw=[%02X,%02X,%02X,%02X]",
             matrixI2CRxPacketCount,
             matrixLampWriteBurstCount,
...
extern "C" void app_main(void) {
    ...
    while (true) {
        // Service and drain all pending I2C packets every loop so the readback
        // register window stays aligned with the control board's pointer writes.
        serviceI2C();
        ...
        const bool completedLampFrame = refreshLampMatrixStep();
        if (completedLampFrame && !MATRIX_DIAG_SKIP_SWITCH_SCAN) {
            scanSwitchMatrix();
        }
