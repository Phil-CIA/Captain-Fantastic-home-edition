...
void refreshLampMatrixStep() {
    ...
    if (!matrixOutputEnabled) {
        ...
        row = static_cast<uint8_t>((row + 1) % CAPTAIN_SWITCH_ROWS);
        if (MATRIX_INTEGRATED_SWITCH_SCAN && !MATRIX_DIAG_SKIP_SWITCH_SCAN && row == 0) {
...
// Optional decoupled switch scan path. This avoids sampling switch columns during the
// active lamp refresh row and is useful when investigating comm/scan-coupled ghost inputs.
...
extern "C" void app_main(void) {
    ...
    while (true) {
        ...
        if (!MATRIX_INTEGRATED_SWITCH_SCAN && !MATRIX_DIAG_SKIP_SWITCH_SCAN && i2cServiceRowCounter == 0) {
            scanSwitchMatrix();
        }
